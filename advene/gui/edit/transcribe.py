...
class TranscriptionEdit(AdhocView):
    ...
    def populate(self, annotations):
        ...
        # FIXME: could be optimized through a specify model call
        l=[ (a.begin, a.end, a) for a in annotations ]
        last_end=-1
    ...
    def import_annotations_cb(self, button=None):
        ...
        for a in at.annotations:
            if a.begin > last_time:
                it=b.get_iter_at_mark(b.get_insert())
                mark=self.create_timestamp_mark(a.begin, it)
            # FIXME: use self.controller.get_title ?
            b.insert_at_cursor(a.content.data)
            it=b.get_iter_at_mark(b.get_insert())
            mark=self.create_timestamp_mark(a.end, it)
            last_time = a.end
        return True
        ...
        while not finished:
            ...
            if res == gtk.RESPONSE_OK:
                ...
                if at == newat:
                    ...
                    at=s.create_annotation_type(id=id_)
